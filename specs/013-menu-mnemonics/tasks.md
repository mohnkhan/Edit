...
- [x] T001 Create the integration test file `tests/integration/menu_mnemonics.rs` with module
...
- [x] T002 Create the smoke test stub `tests/smoke/menu_mnemonics.exp` (expect script skeleton based
...
- [x] T003 [P] Unit tests in `src/ui/menubar.rs` (`#[cfg(test)]`) for `auto_mnemonic(label, &mut used)`:
  ...
  wide label).
- [x] T004 [P] Unit tests in `src/ui/menubar.rs` for `underline_col(label, mnemonic)`: returns the
...
- [x] T005 [P] Unit tests in `src/ui/menubar.rs` for built-in authored mnemonics: every built-in item
  ...
  sets).
- [x] T006 Add `mnemonic: Option<char>` to the static `MenuItem` struct and populate every entry in
...
- [x] T007 Add `mnemonic: Option<char>` to `ResolvedItem` and `ResolvedMenu` in `src/ui/menubar.rs`.
- [x] T008 Implement `auto_mnemonic(label: &str, used: &mut std::collections::HashSet<char>) ->
  Option<char>` in `src/ui/menubar.rs` per data-model.md.
- [x] T009 Implement `underline_col(label: &str, mnemonic: Option<char>) -> Option<u16>` in
  `src/ui/menubar.rs` using the existing `grapheme_width`/char-width logic.
- [x] T010 Update `resolve_menus` in `src/ui/menubar.rs` to populate mnemonics: built-in top-level =
...
- [x] T011 Update all existing `ResolvedItem`/`ResolvedMenu` literal constructions (tests + non-test
...
- [x] T012 [P] [US1] Unit test in `src/ui/menubar.rs`: rendering the bar underlines exactly the
  first letter of each top-level title (F/E/S/V/O/H) and nothing else on the bar row.
- [x] T013 [P] [US1] Unit test in `src/ui/menubar.rs`: with the File dropdown open, each item row has
...
- [x] T014 [US1] In `MenuBarWidget::render` (`src/ui/menubar.rs`), add `Modifier::UNDERLINED` to the
  accelerator cell when drawing each top-level label, using `underline_col` + the menu's `mnemonic`.
- [x] T015 [US1] In `MenuBarWidget::render`, add `Modifier::UNDERLINED` to the accelerator cell when
...
- [x] T016 [P] [US2] Unit test in `src/ui/menubar.rs` for `select_item_by_mnemonic(menus, ch)`:
...
- [x] T017 [US2] Implement `MenuBarState::select_item_by_mnemonic(&mut self, menus, ch) ->
  Option<Action>` in `src/ui/menubar.rs` per data-model.md.
- [x] T018 [US2] In the menu-active keyboard intercept in `src/app.rs` (`handle_action`), add an
...
- [x] T019 [P] [US2] Integration test in `tests/integration/menu_mnemonics.rs`: open each built-in
...
- [x] T020 [P] [US3] Unit test in `src/ui/menubar.rs` for `open_menu_by_mnemonic(menus, ch)`: from
...
- [x] T021 [US3] Implement `MenuBarState::open_menu_by_mnemonic(&mut self, menus, ch) -> bool` and
  extend the `src/app.rs` `InsertChar(c)` intercept so that in `TopActive` it calls this (consume
  regardless of match).
- [x] T022 [P] [US3] Unit test in `src/input/mod.rs`: `dispatch_key` maps a `Press` of
...
- [x] T022a [P] [US3] Unit test in `src/ui/menubar.rs` (FR-005/SC-002 consistency): for each built-in
...
- [x] T023 [US3] In `src/input/mod.rs` `dispatch_key`, add the lone-`Alt` → `Action::Menu` mapping
  (before the char fallback), guarded to `KeyEventKind::Press`/`Repeat` only.
- [x] T024 [US3] In `src/app.rs` terminal init/teardown, if `supports_keyboard_enhancement()` is
...
- [x] T025 [P] [US4] Unit tests in `src/ui/menubar.rs`: (a) two plugin items merged into Edit get
  ...
  without panic.
- [x] T026 [US4] Integration test in `tests/integration/menu_mnemonics.rs`: build `App` resolved
...
- [x] T027 Fill in `tests/smoke/menu_mnemonics.exp`: launch editor, `F10`, open File, press an item
  letter, then `Ctrl+Q`; assert clean exit 0 (mirror `tests/smoke/file_browser.exp` structure).
- [x] T028 [P] Update `CHANGELOG.md` under `[Unreleased]` with a `feature 013` entry (Added:
  underlined accelerators on all menus/items; letter activation in dropdowns; bare-Alt activates bar
  best-effort).
- [x] T029 [P] Update `docs/STATUS.md` with feature-013 user-story rows (US1–US4, Complete).
- [x] T030 [P] Update `docs/CAPABILITIES.md`: Menu keybindings section — note underlined accelerators,
  letter-to-activate inside an open menu, and bare-Alt (terminal-permitting) bar activation.
- [x] T031 Run the full local gate: `cargo fmt`, `cargo clippy --all-targets -- -D warnings`,
