...
fn typing_edits_path_while_browser_focused() {
    ...
    assert_eq!(a.file_browser.as_ref().unwrap().filename, "xy");
    let _ = fs::remove_dir_all(&base);
}
...
fn arrows_noop_while_button_focused() {
    ...
    let sel = a.file_browser.as_ref().unwrap().selected;
    a.handle_action(Action::MoveDown).unwrap(); // no-op while a button is focused
    assert_eq!(a.file_browser.as_ref().unwrap().selected, sel);
    let _ = fs::remove_dir_all(&base);
}
...
fn entry_double_click_still_activates() {
    ...
    let fb = a
        .file_browser
        .as_ref()
        .expect("stays open after entering dir");
